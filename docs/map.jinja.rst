...
We will see a quick setup to configure the ``sonarr`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``sonarr`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``sonarr`` formulas under ``/srv/salt/sonarr/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/sonarr/parameters/dns:domain/
    ln -s dns:domain /srv/salt/sonarr/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/sonarr/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/sonarr/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``sonarr`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``sonarr`` formulas under ``/srv/salt/sonarr/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/sonarr/parameters/roles
...
We create a configuration for the role ``sonarr/server`` in ``/srv/salt/sonarr/parameters/roles/sonarr/server.yaml``:
...
We create another configuration for the role ``sonarr/client`` in ``/srv/salt/sonarr/parameters/roles/sonarr/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles sonarr/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles sonarr/client
...
.. code-block:: console
    ...
    mv /srv/salt/sonarr/parameters/dns:domain/  '/srv/salt/sonarr/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-sonarr-as-json:
      test.nop:
        - name: {{ sonarr | json }}
...
Here is an example based on `template-formula/sonarr/config/file.sls`_:
    ...
    sonarr-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='sonarr-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ sonarr.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``sonarr`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ sonarr['winner'] }}
...
.. _template-formula/sonarr/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/sonarr/config/file.sls
